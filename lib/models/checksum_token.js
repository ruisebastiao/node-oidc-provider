...
module.exports = function extendBaseToken(provider) {
  ...
  return class ChecksumToken extends BaseToken {
    ...
    static async validateStored(tokenValue, stored, { ignoreExpiration }) {
      const jwt = [stored.header, stored.payload, stored.signature].join('.');
      const { payload } = JWT.decode(jwt);
      assert(constantEquals(Buffer.from(tokenValue.substring(59)), Buffer.from(stored.signature)));
      assert(tokenHash(payload.checksum, tokenValue.substring(48, 59)));
