...
module.exports = function getBaseToken(provider) {
  ...
  return class BaseToken {
    ...
    async save() {
      const [upsert, tokenValue] = await this.getValueAndPayload();
    ...
    static async find(token = '', { ignoreExpiration = false } = {}) {
      try {
        ...
        assert(stored);
        const payload = await this.validateStored(token, stored, { ignoreExpiration });
     ...
     * @api public
     */
    static async validateStored(token, stored, { ignoreExpiration }) {
